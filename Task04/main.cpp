...
Graph tasksGraphGenerator(const bool& only_zeros, const size_t& no_tasks)
{
	if (no_tasks < 1)
	{
	...
	for (size_t i = 1; i < no_tasks; i++)
	{
		...
		if (only_zeros)
			graph.push(std::pair<GraphNode*, int>(new GraphNode(i), 0), taskNumber(gen));
		else
			graph.push(std::pair<GraphNode*, int>(new GraphNode(i), dataTransfer(gen)), taskNumber(gen));
	}
...
auto main() -> int
{
	...
	// user input
	auto tasks = tasksGraphGenerator(only_zeros, no_tasks);
	// user input
